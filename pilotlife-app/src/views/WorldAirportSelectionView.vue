    ...
    <div class="selection-panel">
      <div class="panel-header">
        <div class="world-context">
          <span class="world-badge" :class="worldStore.currentWorld.value?.difficulty.toLowerCase()">
            {{ worldStore.currentWorld.value?.difficulty }}
          </span>
          <span class="world-name">{{ worldStore.currentWorld.value?.name }} World</span>
        </div>
        <h1 class="panel-title">Select Your Home Airport</h1>
        <p class="panel-subtitle">This will be your starting location in this world</p>
      </div>
...
onMounted(() => {
  // Ensure we have a current world selected
  if (!worldStore.currentPlayerWorld.value) {
...
async function confirmSelection() {
  if (!selectedAirport.value || !worldStore.currentPlayerWorld.value) return

  saving.value = true

  try {
    const result = await worldStore.setWorldHomeAirport(selectedAirport.value.id)
